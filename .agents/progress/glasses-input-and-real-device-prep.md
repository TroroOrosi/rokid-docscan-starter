...
### 2026-09-12 訂正: この日の端末内推論は本筋ではなく FS-55 の側道

同日の上の節は、端末内モデルを主力のように書いている。これは誤りである。
`tasks/plan.md#glasses-autoscan-listening-20260906`（2026-09-07 更新）が
本筋を次のように定めている。

- 「基本構成はグラス＋ポケット内スマホ＋スマホの4G/5G経由のAIサービス」(plan.md:349)
- 「前回草案の完全オフライン解答必須・スマホ内の大きなモデルを中心とする方針を撤回する」(plan.md:315)
- 端末内AIは**追加候補**(plan.md:361)
- 「PCや利用者が運用する専用サーバは現場構成に含めない」(plan.md:316)

したがってこの日の作業は **FS-55「スマホ内AIによる非API・圏外経路を評価」**
に相当する。FS-55 の完了条件は「完全ローカル解答は実機品質基準を満たした
場合のみ有効にする」であり、測定結果は基準を満たさない。**結論は
「完全ローカル解答は有効化しない」**で、todo.md の FS-55 に記録した。

この日に本筋から外れた点を記録する。次に読む者が繰り返さないため。

1. 側道を本線として扱い、難問対応の設計済みルートである FS-63（GPT中継の
   認証と利用境界）/ FS-64（一大問をGPTで解く）を見ずに進めた。難問対応を
   止めているのは技術ではなく課金方針の未確定であり、記録にもそう書かれていた。
2. **グラスを Wi-Fi でスマホへ直結して通し試験を組んだ。** plan は
   「グラスはWi-Fiに接続せず」「Bluetooth/CXR転送が第一候補」としている。
   192.168.0.4 のグラスから 192.168.0.30 のスマホ API を叩く構成は現場で
   成立しない。評価としては有効だが、現場構成の検証ではない。
3. PC 上で 35B 級 MoE を動かす案を提示した。plan.md:316 に反する。取り下げ。
4. Termux を前面に置き、`sshd` を手で再起動させる運用を作った。
   「初回準備後はスマホをロックしたまま操作0回」(plan.md:341) に反する。

なお `app/llm_http.py` と `OPENAI_BASE_URL` 経路（コミット `1714bd2`）は、
OpenAI 互換エンドポイント全般に効くので FS-63 の管理型中継でも再利用できる。
端末内モデル専用の実装ではない。

## Checkpoint — 2026-09-12 夜 解答経路の順位を訂正し、prefill の壁を取り除いた

利用者の指摘により、同日昼の二つの記録を訂正した。

### 訂正1: スマホ内ローカルが本筋である

同日昼の「訂正: この日の端末内推論は本筋ではなく FS-55 の側道」という節は、
それ自体が誤りだった。利用者が渡した設計
（`C:\Users\pupu_\Downloads\F-51F ローカルAI環境構築 Codex用プロンプト.md`、2026-09-12 17:33）は
**スマホ内ローカルモデルで難問を解く**もので、これが本筋である。GPT中継は最終手段として
圏内に残す（勝手に外そうとしたのも誤り）。順位は `tasks/plan.md#answer-route-20260912` に書いた。

2026-09-07 の「スマホ内の大きなモデルを中心とする方針を撤回する」が撤回の対象にしていたのは
**PCや自前サーバを現場構成へ持ち込むこと**であって、スマホ内で動かすこと自体ではない。
この区別を失ったために、PC 上の FastAPI から ssh トンネル越しにスマホの `llama-server` を
叩く構成を「接続確認」と記録した。動くが現場では成立しない。PC は APK ビルドとモデル転送だけに使う。

### 訂正2: FS-55 の「完全ローカル解答は有効化しない」を取り消す

根拠が二つとも弱い。

1. 「完全一致 1/6、記述式17件中5件が空文字」という数字は `tasks/todo.md` にしかなく、
   **実行手順がこの進捗記録に残っていない**。再現できない。空文字はモデルの能力ではなく、
   生成打ち切りか出力解析の失敗の形である。
2. 測定時のサーバは、**小問1問ごとに文書の全ページ OCR を prompt へ入れていた**
   （`_document_material` の docstring が "Return ALL pages of the document"、
   `_exam_prompt_context` が "the **whole document**"）。クラウドの高速 prefill 前提の設計で、
   端末内モデルには成立しない。

### 実証の前に行った計算

- **prefill が律速。** 実測 pp128 = 19.65 t/s（`Qwen3-4B-Instruct-2507` Q4_K_M）。
  1ページ 600-1,200字を 500-1,000 トークンとすると 20ページで約16,000トークン、
  1問あたり約814秒 = 13.6分。小問20問で約4.5時間。保守的に1ページ300トークンとしても
  1問5分・20問100分で、いずれにせよ150分の試験中には終わらない。
  **生成速度 5.87 t/s だけを見ていたのが前回の誤り。**
- 大問単位（約2,000トークン）に絞れば1回102秒。大問6つで約10分に収まる。
- **KV メモリ。** 1トークン = `2 × 層数 × KVヘッド数 × head_dim × 2バイト`。
  Qwen3-4B を層36 / KVヘッド8 / head_dim 128 と仮定すると 144 KiB/token。
  `-c 2048` で288MB、`-c 8192` で1.15GB、`-c 32768` で4.6GB。`-c` 未指定だと
  llama.cpp は学習時コンテキスト（Qwen3-4B-2507 は 262,144）を確保しようとする。
  同日昼の「既定コンテキストで kill」「9B がロードできない」はこれで説明がつく。
  層数等は GGUF metadata で確認すること（FS-68）。**9B の不採用は保留に戻した。**
- **時間予算。** 全問一括なら分析10分で約3,500トークンしか作れない。契約4の
  「検査を通った小問から保存」を守り閲覧130分も生成に使えば約45,000トークン。

### 実装した変更（`7d88b88`、branch `agent/group-scoped-solver-context`）

各小問へ渡す文脈を、その小問が属する大問のページだけに絞った。新しい仕組みは足していない。
deck の各行は既に `structure_json.page_indexes` に跨るページを持ち、`_answer_groups` が
既に大問へ畳んでいたので、両方を再利用した。

- `_document_material(conn, doc_id, current_index, page_indexes=None)` に絞り込みを追加。
- `_row_page_indexes(row)` / `_group_page_indexes(conn, session_id)` を追加。
- solve ループ（`finalize-reading` のサーバ側一括解答）が大問のページ集合を渡す。
- 大問見出しが無い文書は1グループに畳まれ、従来の全ページ動作のまま。
- 互換経路 `solve-current` は変更していない。ここは今も全ページを送る。

実行した検証:

```
py -3.12 -m pytest -q          -> 473 passed, 1 warning in 54.83s
py -3.12 -m ruff check .       -> All checks passed!
```

新テスト `test_finalize_reading_scopes_solver_context_to_its_own_group` は、
絞り込みを外すと `assert '【P03' not in ...` で落ちることを確認済み（空振りしていない）。

### 次（FS-67 → FS-68 → FS-69）

1. 実際の試験ページのOCRを対象modelのtokenizerで数える。合成fixtureでは代用しない。
2. GGUF metadata から KV 係数と許容 `-c` を4B/9Bで算出する。実機ロードはその後。
3. 大問スコープ後の答案品質を、再現できる手順を記録したうえで測り直す。

未着手のまま残っているもの: Termux 上での FastAPI 常駐、グラス⇄スマホを Bluetooth/CXR へ
...
## 2026-09-12 夜 9B は `-c 8192` でも落ちる / 実資料のページ規模を測る

### 9B ロード試験: 失敗。計算は必要だったが十分ではなかった

利用者の承認を得て `-c 8192` でロードした。

```
llama-cli -m Qwen3.5-9B-Q4_K_M.gguf -c 8192 -t 4 -n 32 -st --no-warmup -p "2+3="
```

結果は前回と同じ kill 連鎖で、sshd ごと落ちた。`adb logcat -b events` に残った証拠:

```
09-12 22:49:23.158 am_proc_died: [0,15707,com.fujitsu.mobile_phone.fjhome,100,2]
09-12 22:49:23.340 am_proc_died: [0,17608,com.termux,0,2]
```

ロード直前の MemAvailable は 6,992 MB（`~/9b_mem.log` の1点目）。計算上の所要は約5.6 GiBで
収まるはずだったが収まらなかった。**KV の計算は「既定 `-c` でなぜ死ぬか」の説明としては
正しいが、「`-c` を直せば載る」の根拠にはならない。** MemAvailable の大半は回収可能な
page cache であり、5.29 GiB の重みを mmap で読む圧力自体が LMK を起動させる。

どの段階で落ちたか（重みロード中か KV 確保時か）は `~/9b_test.log` に残っているが、
Termux の private ディレクトリなので sshd 復旧まで読めない。**未解決。**

次に試す価値がある手（いずれも未実施）: `-ctk q8_0 -ctv q8_0` でKVを半減、`--no-mmap` の
有無を比較、より小さい量子化、常駐アプリを減らした状態での再試験。
**4B は載ることが既に分かっているので、9B は必須ではない。**

### 実資料のページ規模（2026年度 共通テスト、公式PDF）

`data/docscan.db` に実撮影ページが無いため、公式の問題PDFからページ規模を測った。
`pdftotext` は Adobe-Japan1 の CMap を持たず文字化けする（`pdffonts` で確認: CID Type 0C /
Identity-H / `uni=no`）。CMap を同梱する `pdfminer.six` で抽出した。

| 科目 | ページ | 総文字数 | 日本語文字 | 文字/ページ | 中央値 | 最大 |
|---|---|---|---|---|---|---|
| 数学ⅠA | 26 | 12,955 | 6,555 | 498 | 480 | 739 |
| 国語 | 47 | 65,189 | 25,897 | 1,387 | 1,179 | 2,639 |
| 英語リーディング | 31 | 35,709 | 825 | 1,152 | 1,131 | 1,962 |

**このPDFには抽出禁止フラグが立っている。ページ規模の計測にのみ使い、抽出テキストは
リポジトリに保存しない。** 実撮影のOCRはレイアウト由来の差が出るので、この数値は
下限側の目安として扱う。

### ここから引ける時間予算（Qwen3-4B-Instruct、0.77 トークン/文字、pp 19.65 t/s）

| | 全文トークン | 全ページを毎問（旧実装） | 大問単位（`7d88b88` 後） |
|---|---|---|---|
| 数学ⅠA 全体 | 約9,975 | 小問20問で約2.8時間 | 大問1つ約1,900トークン = 約98秒 |
| 国語 全体 | 約50,200 | 実行不能 | 大問1つ約12,800トークン = 約11分 |

数学は大問単位なら収まる。**国語は大問1つで11分かかり、逐次生成前提でも重い。**
科目によって成立条件が違うことが数字で出た。英語リーディングは日本語文字が825文字しか
無く英語主体なので、トークン比率が違う（未測定）。

**未測定:** 実ページのトークン数そのもの。sshd 復旧後に `llama-tokenize` で科目別に測る。

### 9B が落ちた地点（sshd 復旧後に読んだ）

`~/9b_test.log` の末尾は `Loading model... |-\|/...` で終わっている。
**KV 確保まで到達しておらず、重みの読み込み中に落ちた。** つまり KV の計算はこの失敗とは
関係がない。`~/9b_mem.log`:

```
22:49:13 6969MB  49:15 6900MB  49:17 7225MB  49:19 6526MB  49:21 5172MB  -> kill
```

**MemAvailable が 5.1GB 残っている時点で kill されている。** 単純な枯渇ではなく、
原因は未特定のまま。4B は載るので 9B は本筋に不要であり、ここで打ち切る。
再開するなら `--no-mmap` の有無の比較から。

### FS-67 完了: 科目別の実トークン数（`llama-tokenize`、Qwen3-4B-Instruct-2507）

公式PDFから `pdfminer.six` で抽出した本文を、実際にトークン化した。

| 科目 | 総トークン | ページ | トークン/ページ | トークン/文字 |
|---|---|---|---|---|
| 数学ⅠA | 9,536 | 26 | **367** | 0.74 |
| 国語 | 61,953 | 47 | **1,318** | 0.95 |
| 英語リーディング | 9,395 | 31 | **303** | 0.26 |

**科目差が大きい。** 国語は数学の3.6倍。英語リーディングは英文主体で0.26トークン/文字と安く、
日本語の比率（0.77）をそのまま当てると3倍近く過大評価になる。科目ごとに測る必要がある。

注意: 端末の `wc -m` はロケール未設定でバイト数を返す。文字数は PC 側の値を使うこと。

### prefill 速度はプロンプト長で崩れる（これが本当の制約）

`llama-bench -p 128,512,2048,8192 -n 0 -t 4 -r 1`（Qwen3-4B-Instruct-2507 Q4_K_M）:
...
| pp512 | 14.52 | 78% |
| pp2048 | **7.05** | 38% |
| pp8192 | 測定中（CPU時間 1:52:59 経過時点で未完了） | — |

**これまで使ってきた 19.65 t/s は `pp128` の値で、実際の見積りには使えない。**
2,048トークンで既に 7.05 t/s、つまり 2,048トークンの prefill に 290秒 = 4.8分かかる。
pp8192 は CPU時間で2時間近く回しても終わらない。

この測定によって、先に書いた時間予算はすべて楽観側だったことが分かる。実測に基づく修正:

| | トークン | prefill（7.05 t/s で計算、実際はこれより遅い） |
|---|---|---|
| 数学ⅠA 大問1つ（約2,400） | 2,400 | 約5.7分 |
| 数学ⅠA 全体 | 9,536 | 約23分（大問4つ、KV再利用を仮定） |
| 国語 大問1つ（約15,500） | 15,500 | **37分以上**（pp8192未満の速度で計算した下限） |

**国語は端末内 4B では成立しない。** 数学ⅠA は大問単位＋逐次生成なら成立の可能性が残る。
科目によって可否が分かれることが、見積りではなく実測で確定した。

これは `7d88b88` の大問スコープ化が必要だったことを裏付けると同時に、それだけでは
不十分であることも示している。次に検討すべきは入力そのものの削減（小問に関係する
本文だけを渡す、OCR全文ではなく構造化した抜粋を渡す）であって、モデルの差し替えではない。

### 訂正: 先に報告した国語のページ規模は抽出フォーマットの産物だった

`pdfminer.six` は縦書きの国語を**1文字ごとに改行**して出力し、段組の読み順も混ざる
（`(cid:7923)` 等の未マップ文字も残る）。この状態でトークン化していたため、
1文字ごとの改行がトークンの結合を妨げ、改行自体もトークンになっていた。

空白と cid マーカーを除去した実文字数:

| 科目 | レイアウト込み | **実文字数** | ページ数 | 実文字/ページ |
|---|---|---|---|---|
| 数学ⅠA | 9,946 | 7,992 | 26 | 307 |
| 国語 | 55,387 | **28,325** | 47 | **603** |
| 英語リーディング | 34,660 | 28,159 | 31 | 908 |

**先に記録した「国語 1,387文字/ページ、61,953トークン」は誤りである。** 実際の本文量は
その半分以下で、国語の総文字数は 28,325。0.95トークン/文字という値も抽出の産物であり、
本文の性質ではない。

抽出テキストは読み順が壊れているため**解答能力の検証には使えない**。ページ規模の
目安にのみ使う。実機OCRは読み順が正しいので、実際の値はここからさらに変わりうる。

トークン数の測り直しは端末のベンチマーク終了後に行う。

### 国語の形式から来る設計上の事実

公式正解（`R8_【国語】発表用正解.pdf`）を確認した。**共通テスト国語は5大問37解答番号が
すべてマーク式**（200点満点、第1問45/第2問45/第3問20/第4問45/第5問45）。したがって:

- **生成トークンはほぼゼロ。** 国語のコストは事実上100%が prefill である。
- 採点は完全一致で客観的に行える。解答能力の検証材料として使える。

### 実トークン数（空白除去後）と、長文 prefill のモデル間比較

`llama-tokenize`、空白と `(cid:N)` を除いた本文:

| 科目 | 実文字数 | Qwen3-4B-Instruct | **Qwen3.5-4B** | 旧記録（誤） |
|---|---|---|---|---|
| 数学ⅠA | 7,992 | 6,255 | **5,744** | 9,536 |
| 国語 | 28,325 | 22,480 | **20,889** | 61,953 |
| 英語リーディング | 28,159 | 8,651 | **8,548** | 9,395 |

**国語は旧記録の3分の1。** 実機OCRは行単位の改行が入るので実際はこれより少し多く、
47ページ×約30行として約1,400改行を足した 22,000〜23,000トークンが現実的な目安。

`llama-bench -p 512,2048 -n 0 -t 4 -r 1`:

| モデル | pp512 | pp2048 | 2048/512 |
|---|---|---|---|
| Qwen3-4B-Instruct-2507 (qwen3) | 14.52 | 7.05 | 49% |
| **Qwen3.5-4B (qwen35)** | 15.61 | **11.58** | **74%** |
| Qwen3.5-0.8B (qwen35) | 71.09 | 49.15 | 69% |

**長文 prefill では Qwen3.5-4B が Qwen3-4B-Instruct の1.64倍速い。** `full_attention_interval=4`
で32層中8層しか full attention を持たない構造が、長文で効いている。KV も 32KiB/token と
4.5分の1で、長文保持の両面で有利。

**訂正:** 2026-09-12 昼に「thinking を出さないので既定の解答エンジンを
`Qwen3-4B-Instruct-2507` にした」と記録したが、**長文を読む用途ではこの選択は誤り**である。
thinking の抑制は `/no_think` 等のプロンプト側で扱い、長文前提のモデルは Qwen3.5 系を採る。

熱は除外済み（測定中も全ゾーン `mStatus=0`、SKIN 36.1℃）。pp の低下は熱ではなく処理特性。
Vulkan バックエンドは未ビルド（`build/bin` に `libggml-vulkan.so` が無い）。CPU のみ。

## 2026-09-13 スマホ内 4B が共通テストを実際に解いた（英語リーディング第2問 満点）

### 解答能力: 4/4 正解

`Qwen3.5-4B-Q4_K_M`、F-51F 端末上、`llama-cli -c 4096 -t 4 -ub 256 -n 400 -st
--reasoning off`。入力は2026年度 共通テスト 英語リーディング 第2問の本文＋設問
（公式PDFから `pdfminer.six` で抽出、約950トークン）。`expected` は一切渡していない。

| 設問 | 解答番号 | 公式正解 | Qwen3.5-4B | 判定 |
|---|---|---|---|---|
| 問1 | 4 | 1 | 1 | ○ |
| 問2 | 5 | 3 | 3 | ○ |
| 問3 | 6 | 4 | 4 | ○ |
...
`[ Prompt: 15.7 t/s | Generation: 4.8 t/s ]`

正解は公式の `R8_【英語（リーディング）】発表用正解.pdf`（解答番号1-21 =
3,1,2,1,3,4,2,2,2,3,5,1,4,3,3,1,3,3,4,5,3）から取った。設問直後に現れる数字が
解答番号であることは抽出本文（「問 ２ … / ５」）で確認した。

**これは「スマホ内モデルでは解けない」という 2026-09-12 の見方を直接否定する。**
FS-55 で記録した低い正答率は、合成fixtureに対する再現不能な測定であり、
実際の試験問題に対する能力を示していなかった。

### thinking の抑制は `--reasoning off`

`/no_think` をプロンプトへ書いても Qwen3.5 は thinking を出し続け、`-n 200` では
解答に到達しなかった（thinking 途中で打ち切り）。`llama-cli --reasoning off`
（`-rea off`）で抑制できる。`--jinja` が既定で有効なので chat template 経由で効く。

**これは 2026-09-12 に「thinking を出すから Qwen3.5 ではなく Qwen3-4B-Instruct を
既定にする」と判断した理由を無効にする。** 抑制はフラグ一つで済む。

### 長文コンテキストは速度と正答率の両方を落とす（大問スコープ化の実証）

同じ4問（英語リーディング 第2問 問1-4、公式正解 1/3/4/2）を、与える文脈だけ変えて比較した。
モデル・設定は同一（`Qwen3.5-4B-Q4_K_M`、`-t 4 -ub 256 -st --reasoning off`）。

| 条件 | 入力トークン | `-c` | prompt t/s | generation t/s | 解答 | 正答 |
|---|---|---|---|---|---|---|
| 第2問のみ | 約950 | 4096 | **15.7** | 4.8 | 1,3,4,2 | **4/4** |
| 全問題を文脈に | 8,548 | 16384 | **6.2** | 2.9 | 1,3,2,2 | **3/4** |

**長文にすると prompt 速度は 39% へ落ち、正答も1問失う。** 該当箇所を見つける負荷が
正答率に出ている。`VmRSS` は `-c 16384` で 6,037,584 kB = 5.75GB。

**これは `7d88b88`（解答プロンプトを大問単位に絞る）を実測で裏付ける。** 大問スコープ化は
速度のための妥協ではなく、正答率のためにも正しい。全ページを毎問渡す旧実装は両方で劣る。

この数値で引き直した科目別の見通し（大問単位、KV前置き共有あり）:

| 科目 | 総トークン | 大問数 | 大問あたり | 速度の目安 | 合計prefill |
|---|---|---|---|---|---|
| 数学ⅠA | 5,744 | 約4 | 約1,400 | 15 t/s | 約6分 |
| 英語リーディング | 8,548 | 8 | 約1,070 | 15.7 t/s | 約9分 |
| 国語 | 20,889 | 5 | 約4,200 | 9-10 t/s（内挿、未実測） | 約37分 |

**国語も大問単位なら150分に収まる。** ただし大問あたり4,200トークンは、正答率が落ち始める
8,548トークンと950トークンの中間にある。**国語の正答率は実測が要る**（FS-71、縦書きのため
実機OCRか別経路の本文が必要）。

速度の測定点（Qwen3.5-4B、`-t 4`）: pp512 15.61 / pp2048 11.58 / 約950実測 15.7 /
8,548実測 6.2。pp2048 と 8,548 の間で大きく落ちる。

## 再開入口（2026-09-13・この節から読む。上の 2026-09-10 の同名節は古い）
...
## 2026-09-13 縦書き科目の本文取得経路を確立した（評価用）
...
## 引き継ぎ要約（2026-09-13・ここを最初に読む。上の再開入口より新しい）
