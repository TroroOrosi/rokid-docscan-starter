...
- **撮影しない＝その場認識**: カメラに映った資料を搭載 AI が**その場で認識**し
  （視認＝認識）、撮影（写真ファイル生成・フラッシュ・シャッター音）を一切挟まない。
  資料情報の事前収集はしない。サーバは本体 AI の認識テキスト（本文=`ocr_text`＋図の
  読み取り=`vision_text`）のみ受ける。録音も無音。撮影を挟まないためフラッシュ不要。
- 照合（`/v1/match`）も**テキスト照合が主**（pHash 無しの側があってもテキストでスコア。
  画像は互換の任意入力・非推奨）。設問取り込み（`/questions`）もテキストのみで完結
  （画像は互換・非推奨）。
...
1. **読取**（カメラ ON・最短化）: `POST /v1/documents` → 2本指タップ（AI 起動=視認）×全ページ
   ...
   **ダブルタップ=読取完了宣言** → `POST /finalize-reading`（問題分割・デッキ作成・
   status open/reading→reviewing・冪等・カメラ OFF）。中断復帰は
   `GET /scan-status?expected_total_pages=N`（欠番報告→欠番のみ再読取。読み取り専用・
   画像/pHash 語彙なし）。
