...
- [ ] 実装・検証完了（2026-09-11、答案バンドルの転送(`GET
  ...
  `.agents/progress/glasses-input-and-real-device-prep.md`の同日付節）
...
- [ ] 部分完了（2026-09-12。トークン化比率は実測: Qwen3-4B-Instruct 0.77 / Qwen3.5-4B 0.70 トークン毎文字。**実ページの文字数は未測定** — `data/docscan.db` の `pages` が0件で実撮影が残っていない）
...
- [x] 計算完了（2026-09-12。qwen3 4B=144KiB/token、qwen35 は `full_attention_interval=4` で8層のみ KV を持ち 32KiB/token。全モデル `context_length=262144` で、`-c` 未指定だと 4B が36GiB・9Bが8GiBのKVを確保する。**9Bの不採用は取り消し**、`-c 8192` で約5.6GiB、空き6.21GiBに収まる見込み）
