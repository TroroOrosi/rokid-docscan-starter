...
Runs on: Windows PCのみ。実機・導入・GPT送信停止を維持する。最新方針はplanの「目的と手段の再整理」。

- [x] GPTへのOCR全文・補正文・ASR文字起こし入力を除去。全画像・原音を維持し、OCR範囲で共通資料を落とさない。
- [x] 主経路のASR設定・推論・完了待ちを解除。保存、欠番・hash・sample検査、再送、完了要求は保持する。
- [x] 同一会話で確認済みなら添付画像を再生成しない。原本／会話変更・未確認は再準備する。
- [x] 原本欠損を無視する別入口と、OCRの選択肢数で原本の解答を再質問する処理を修正。
- [ ] CQ-5〜8: OCRを判読性の指標として校正し、候補保全と正式登録を分離する本流へ接続する。
- [ ] RP-12: 現行のOCR依存の設問分割を正答一覧の根拠にしない。原本上の小問・解答欄との対応を検証する。
- [x] RP-15（PC）: `finalize-reading?solve=background` は分割後に返り、同じclaimで1セッション1本の解答処理を続ける。
  glassdocはPENDINGが残る間だけ5秒ごとにbundleをGETし、`AnswerReader.accept`と保存へ反映する。再送はしない。
- [x] RP-12a（一部）: 問N直下の行頭`(A)`等の英字は選択肢として保持し、偽の小問を作らない。大問直下・`(1)`は従来どおり区切り。
- [x] RP-12（PC）: background解析はsolverに原本から小問一覧（大問・ラベル・ページ）を1回作らせ、deckにする。
  一覧が不正ならOCR分割、送信結果不明なら以後送信しない。大問見出し行は解かず、設問指定は「第2問 問1」。利用者決定（2026-09-23: OCRよりモデルが正確）。
- [ ] RP-12実測: 実冊子でモデル一覧の過不足・ページ対応・解答欄単位との一致。未測定。
- [ ] RP-15残り: サーバ再起動で解答処理が消えたPENDINGの再開手段、実機でのポーリング電力・AP通信量。
- [ ] RP-01/13/22: 通常姿勢の文字判読、GPTの原音利用と正答、AP経路、復旧・長時間運用を未測定条件ごとに評価。

上の実装修正は撮影品質・全小問解答・会場受け入れの完了ではない。旧ASR必須／OCR資料優先は撤回する。

**進め方の訂正（2026-09-23）:** 9/22〜23はTesseractの文字比較（33→109文字）と文書再整理が続き、
実機もGPTも要らない本流の欠陥（RP-15、RP-12a、CQ-8の登録境界）が未着手だった。
...
Runs on: 今回はWindows PC。実機試験・導入・実資料外部送信は停止を維持する。

- [x] CQ-1: 文字枠だけの判定、旧COMPLETE復元、欠け順位・非有限confidence、構図確認のHUD。
- [x] CQ-2: 保存原本の全域1対1タイル、別保存の非生成明るさ補正、既存geometryとの接続。
- [x] CQ-3: 出所とprofileを検査する登録条件評価、分割間混入を拒否する単頁／見開き比較部品。
- [x] CQ-4: 保存済み4原本でパケット生成、前後hash照合。画質合格・校正済みとはしない。
- [ ] CQ-5 / RP-01/10: 原本から紙面・細字・数式・図表の証拠を生成し、参照資料／人手ラベルで校正する。
  PCの3領域109文字比較は記録済み。補正・単純分割は採用せず、原寸の誤認識も残る。
  [比較結果](../docs/rokid-capture-research.md#2026-09-23-3領域で縮小補正分割を比較)は探索資料で、ML Kit評価・全文校正・登録ゲートの完了ではない。
- [ ] CQ-6 / RP-09/10: 撮影前の新鮮なpreview、四隅・余白・位置安定、同時出力とメモリを検証する。
- [ ] CQ-7 / RP-06/09/11: 本撮影後に同一原本を再検査し、capture_id/hash/rotation/policy/profile版を結ぶ。
- [ ] CQ-8 / RP-06/09/11: 手動・自動・終了・復元の全経路で候補保全と正式登録を分離し、
  confirm/uploadキュー/サーバ登録へ同じゲートを接続。遅延・timeout・不明はhold、3秒無操作を品質承認にしない。
...
## 席の制約・判読・全体の接続（2026-09-22の訂正、上の全体整理へ統合）

Runs on: Windows PCの保存原本と実ソース。追加撮影・APK更新・通常読取・GPT送信は停止。

5方向の点検結果は [撮影研究](../docs/rokid-capture-research.md)、証拠は既存進捗へ集約する。
新しい診断基盤を足す前に以下を既存CQ/RPへ戻す。いずれも実装・実機合格にはしない。

1. CQ-5 / RP-10: 後退・正確な測距を前提から外す。8枚の距離・姿勢は未確認、最新2枚も着席基準にしない。
   同じ原本の必要範囲と細字を比べ、全画面縮小で失う情報と、原本から既に読めない情報を分ける。
2. CQ-6〜8 / RP-09/10: 表示だけのguide/spreadを撮影品質と混同しない。紙面以外のOCR文字数による順位、
   自動OCRゼロ時の候補破棄・反復、3秒後の品質未検査送信を修正対象にする。図ページも保全する。
3. RP-11/12: 必要な共通資料の確認、過去ページ訂正、選択肢(A)(B)の偽問題化を先に解消する。
4. RP-15: 全問の最終化HTTP応答待ちと1回だけのbundle取得を接続し直し、既存の小問別保存を表示へ届ける。
5. CQ-9: 上記と送信しない測定経路を準備してから、通常姿勢での標準／Camera2比較を具体化する。
   収まらない場合は対応範囲内の倍率・cropと単頁を検討し、単に「離れて」「もう2枚」と案内しない。

## PR37後の実装: 画像比較と露出準備（2026-09-23）
...
- [ ] 小さい大問で全画像＋原音が届き、原音が必要な設問に利用できるか測る。
- [ ] グラスcamera/micを5分同時取得し、元サンプル連続性と写真待ち・送信負荷を測る。
...
- 部分測定: F-51Fの既存adbから閉→開時にchooserを起動するscriptを実装。利用者が1回の物理開閉で
  選択画面への復帰を確認。通信断中の開閉・着脱だけ・CLOSED後・AP・反復・時刻目標は未検証（hardware-measurements §K）。
...
- [x] カメラ／マイクは選択前に開始しない。通信確認を二択やローカル取得の待ち条件にしない。
  通常撮影・chooser・録音開始はHTTPを待たず、RECは有効サンプル後。自動試験で確認、起動時間の物理測定は未実施。
...
Runs on: Windows、グラス録音 → F-51F音声API。

- [x] 現行WAV／30秒chunk／重複1秒を再利用し、書込み中断の末尾回収と未ACK再送を可能にする。
  保存・回収・再送とpending写真からの音声復元を自動試験で確認。UUID配下で接続先混入を防止。物理kill試験は未実施。
- [ ] sync間隔・原音保全・空き容量を測る。容量不足は写真から止める。転送・解析失敗でも原音を残す。
- 検証: 逆順／同一再送／違うhash／欠番／短い末尾、保存・ACK前後kill、original PCM一致、S22/23/30。
- 依存: RP-03。対象: ListeningRecorder、listening API、既存録音／API試験。主経路のACKは原音保存で返す。

### RP-08 実録音開始と二段階終了を画面状態から分ける（M）

Runs on: Windows、glassdoc。入力停止監視は実機確認を併用。

- [x] 有効サンプル後だけREC。OS無音化・入力停止／経路変更は検出可能な公開通知と読取り結果で扱う。実機の検出値校正は未実施。
- [x] 第1BACKは撮影終了、第2の別BACKは最後の写真確認中でも音声終了。取り直しで音声を再開しない。最後の写真保持と遅い文書番号後の再送を自動試験で確認。
...
- [ ] 可視ACK・写真世代・取得時刻を照合し、2999msの遅延操作を元写真へ結ぶ。期限後入力は次写真へ流さない。
  通知prefixからの分類待ちとserial遅延の保護を実装・回帰試験。修正版の実機確認と保存後まで未配信の入力復旧は残る。
...
- [ ] REAL_MODE、原音受信、ログイン・CDP・利用可能状態を初回確認。placeholderや「CDP接続可」だけで準備完了にしない。
...
- 検証: AP断／携帯回線断を分ける。再接続、端末再起動、鍵保持、サーバ／原音受信／CDP状態、実際の到達性を記録。
...
Runs on: F-51F Chrome、入力はグラスの実録音・実写真。

- [ ] 数字・否定・末尾の訂正・複数話者・共通会話・読み上げ順逆転を原音からの答案で評価する。
- [ ] ページ／小問と音声根拠を内容で対応し、原音が必要なのに利用できなければneeds_inputにする。
- 検証: 正解と根拠区間、否定／数値の誤認識、未送信原音と終了後待ち。S19・X03。
- 依存: RP-01/07/08/12。対象: listening、solverの資料指定、既存fixture。専用話者分離基盤は作らない。
...
Runs on: 会場経路の同一PNG／実音声、同じF-51F Chrome・モデル。PCの準備は部品作業。

- [ ] 冊子の全画像＋原音を基準にPDF／結合画像と比較。ページ範囲・細部の画素・原音を揃える。
  OCRは判読性の評価に使用し、GPTへの追加テキストにはしない。local ASR比較を主経路の必須作業に戻さない。
...
Runs on: グラス → F-51F AP → 同スマホFastAPI／Chrome → answer-bundle。会場PCなし。

- [ ] 通常20ページを5回、リスニング30分＋20ページを実施。40ページ側の容量と時間も確認する。
- [ ] 終了→再装着を50回。1秒消灯／3秒操作可能は物理結果で判定し、未達を隠さない。
- [ ] 通常／リスニングの150分運用で電池・温度・空き容量・未送信原音・待ち時間・誤操作／欠落を記録。
