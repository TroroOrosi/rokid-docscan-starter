...
- [ ] 部分完了（2026-09-12。公式PDFから実ページ規模を実測: 数学ⅠA 498文字/ページ、国語 1,387、英語リーディング 1,152。トークン化比率は実測: Qwen3-4B-Instruct 0.77 / Qwen3.5-4B 0.70 トークン毎文字。**実ページの文字数は未測定** — `data/docscan.db` の `pages` が0件で実撮影が残っていない）
...
根拠は `.agents/progress/glasses-input-and-real-device-prep.md` の同日節。
