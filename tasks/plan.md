...
### 2026-09-14 追補：実装は chatgpt-web が先行している

上の順位は**現場構成の目標**として維持する。実装の現況はこれと別である。

| 経路 | 現況 |
|---|---|
| `ROKID_SOLVER=chatgpt-web` | **現行の主経路。** 2026-09-13/14 に実装・実測。利用者のログイン済み ChatGPT ウェブセッションを CDP で操作する。API キー不要 |
| スマホ内ローカルAI | 上の順位どおり本筋。測定は `docs/hardware-measurements.md` E 節。現場構成への組み込みは未了 |
