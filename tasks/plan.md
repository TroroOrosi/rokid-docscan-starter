...
**Current approved plan (2026-09-10):** [記入用解答を中心とする改訂](#answer-sheet-20260910)。実装着手済み。下の先行計画と未完了事項は保持し、競合する操作・処理順はこの改訂を優先する。
...
- グラスWi-Fiなし、スマホ4G/5G・ロック中、端末間の近距離通信を維持する。
  現場PC・自前サーバ・テザリングは不要。既存FastAPIは互換・開発評価経路。
...
3. 解答本文は記入内容のみ。選択記号、必要な単位・数値・式、要求された記述を保持する。
   証明/理由の記述が設問要求なら全文が答え。補足解説・根拠・自信度は混ぜない。
   64文字切断を解消し、材料不足は答えとは別の状態にする。
4. 検査を通った小問からグラスに保存する。現在位置を後着結果で動かさず、保存後の
   閲覧に通信を要求しない。スワイプは表示ページ、短タップは大問→小問の一覧。
5. 閲覧中のダブルタップでCLOSEDと閲覧位置を先に永続保存して選択画面へ戻る。
   通信待ちにしない。次回は選択画面から「前回の解答」を任意に再開する。
...
既存FS番号を維持し、FS-12/36/47の記入用全文契約とローカル閲覧から進める。
...
| R11 | 閲覧終了後、次回は直ちにモード選択へ戻る | 通信断・プロセス終了・端末再起動を挟んでも完了済みセッションが復活しない |
...
| 方式 | 位置づけ | 採用条件/制限 |
|---|---|---|
| Firebase AI LogicのAndroid SDK＋Gemini | 第一評価候補。自前サーバ運用不要 | 複数画像＋音声、構造化結果、長時間資料の分割、対象APKのApp Check、モバイル回線の速度・精度をM0で実証 |
