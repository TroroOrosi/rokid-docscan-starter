...
## 目的と手段の再整理（2026-09-23、下の旧方針より優先）

Runs on: Windows PCで実装・保存資料の評価。実機撮影・導入・GPT送信は停止を維持する。

目的は、普段の席で必要資料を判読可能に取得し、解答用紙に書く内容をグラスで確認できること。
OCR・ASR・連写・資料変換の実行自体を達成条件にしない。

- OCRは撮影品質・登録可否を評価する補助情報。全文・補正文をGPTの優先資料にしない。
  実装はJPEG→OCR→確認表示→登録だが、認識完了と判読合格は別で、登録品質ゲートは未接続。
- 主経路へ送る資料は冊子の全画像とリスニング原音。OCRで推定した大問範囲だけに絞って共通資料を落とさない。
  ローカルASRの実行・完了・認識成功を原音保存や解析開始の必須条件から外す。
  GPTが原音を利用できることと正答精度は別途未検証。利用できなければ資料不足を返し、推測で補わない。
- 同じチャットで送信確認済みの資料は、小問ごとに画像変換・再添付しない。
  原本の変更・会話の変更・添付未確認は再利用できない。送信結果不明時の停止は保持する。
- 撮影品質、必要資料、問題分割、答案の逐次表示、終了・再開を一続きに評価する。
  未接続の品質評価を増やす、OCR文字数だけで良否を決める、実機試験で未実装を埋める、という進め方をしない。

今回の修正順は、不要なGPT入力・ASR待ちの除去→添付再生成の除去→入力から表示までの残る成立条件の照合。
撮影品質の校正・正式登録ゲート、OCR依存の問題分割、途中答案取得の未完了をこの修正の完了に混ぜない。
...
## PR37後の最初の実装単位（2026-09-23）

Runs on: 開発・保存写真の比較・ビルドはWindows PC。実機導入・追加撮影・GPT送信は停止のまま。

目的と席の制約は上節を維持する。最初のPRは、画像の比較結果と撮影時の露出準備に範囲を絞る。
根拠は、既存写真の細字が全画面縮小で失われること、旧撮影結果4件がAE=SEARCHINGであること。
暗さの単一原因や、原寸OCRを使えば必ず読めることは未証明なので、両者を同じ修正で決めつけない。

1. 既存標準写真の同じ文字領域で、原寸・半分・拡大補間を比較する。参照転記・入力hash・誤りを私有記録へ残す。
   これはPCの探索比較であり、実ML Kit精度と距離校正にはしない。原本にない細部を生成しない。
2. glassdocのCamera2へ、低解像度の測光ストリームを用いたAE収束待ちを接続する。
   JPEG最大寸法・回転・3枚burst・retry・既存15秒期限は維持。新たな光源・AF・露出補正値を設定しない。
   SEARCHING・不明を成功にせず、期限内に収束しなければ既存UNKNOWN停止へ送る。1要求につきJPEGは1回。
   測光Imageは即解放し、成功・失敗・終了・古いcallbackでも資源を保持しない。画質合格とは別の準備条件。
3. 自動試験とAPK検査後、新PRに実機未検証と残るCQ/RPを明記する。実機比較は今回の作業に含めない。

露出待ちを接続しても、紙面検出・原寸領域OCR・品質登録ゲートは完成しない。自動cropや原寸OCRの
既定化は、紙面境界・文字順・メモリ・実認識の評価後に行う。RP-11/12/15等をこのPRの完了へ混ぜない。
...
初回だけ接続・権限・ChatGPTログイン・API認証を準備する。主経路に端末内ASRの設定は要らない。
