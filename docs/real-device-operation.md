...
リレー自身が表示を押し出した直後の `AI-exit` は自前のechoで、実測1〜51 ms以内に届きます。
`PressGestureInterpreter` は直前のview push から500 ms未満のexitをechoとして捨て、
それ以外を短押しとして扱います。押下時間は取得できないため、`AI-exit` からは
**短押しのみ**を導出します。登録・読取完了・シャッターの確定操作は、2つ目の信号が
実機で確認できるまでスマホ側ボタンに残します。
