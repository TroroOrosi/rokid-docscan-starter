...
4. 用紙まで40〜60cm離し、グラスをタップして `AIMING` の照準を表示する。用紙中心を
   ...
   写真callbackまで動かさない。
5. `CAPTURE_REVIEW` でスマホとグラスの縮小プレビューを確認する。登録はスマホの
   「この写真を登録」で行う。グラスのタップは同じページの `AIMING` へ戻る再撮影準備
   で、そこからもう一度タップすれば撮り直せる。OCRが0文字でも警告を確認して明示
   登録できる。
...
押下時間は取得できないため、`AI-exit` からは**短押しのみ**を導出します。登録と読取
完了はスマホ側ボタンに残します。シャッターだけは `AIMING` 中のタップへ割り当てます。
照準を合わせている最中にスマホへ手を伸ばすと、その照準で決めた構図が動くためです。
...
| `CAPTURE_REVIEW` | 同ページの再撮影準備（`AIMING`へ） | **この写真を登録** / 同じページを撮り直す / 未登録写真を破棄 |
...
前提とする割当（前ページ再撮影準備、登録、読取完了、新規）は、より多くの信号を
配送するファームウェア向けの予備であり、現行ファームではスマホ側が正規の経路です。
...
- スマホの「この写真を登録」が成功した後にだけ `has_image=true` になる。
