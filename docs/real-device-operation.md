...
| Android client / Glasses View contract | `0.3.4` / `1.8.0` |
...
リレーはこれを検出して現在のDocScan画面へ自動復帰します。`0.3.4` からは、
...
0. 撮影に失敗、またはOCRが0文字・OCRエラーだった場合は、待たずに即座
   （300ms後）に撮り直します。何も取得できていない以上、待つ理由がないためです。
   ただし連続5回読めなければ間隔を6秒へ落とします。カメラを連射し続けると
   プライバシーLEDが点灯したままになり、発熱と電池消費も無視できないためです。
1. 同じページを3枚連続で撮影する（間隔900ms）。
...
4. 6秒待って次のページのバーストへ進む。その間に用紙をめくります。
