...
| Android client / Glasses View contract | `0.3.1` / `1.8.0` |
...
5. `CAPTURE_REVIEW` でスマホとグラスの縮小プレビューと**枠内判定**を確認する。
   判定が「不合格」なら用紙が枠から出ているので撮り直す。登録はスマホの
...
不合格でも登録操作は塞ぎません。自動登録しないという既存の契約を保ったまま、
既定の誘導を「撮り直し」へ変えるだけです。文字と縁の接触は用紙外形そのものでは
ないため、余白だけが切れた場合は検出できず、枠いっぱいの用紙では不合格側へ倒れます。
