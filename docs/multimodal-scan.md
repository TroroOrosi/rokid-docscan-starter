...
通常撮影は通信確認を待たずに開始し、写真はグラス内に確定保存してから独立して送ります。
...
録音末尾の回収と未ACK再送は実装しました。最後の写真確認が残る状態からの音声復元はRP-08で接続中です。
...
グラスの`files/local-scans/{ローカル記録ID}/listening-{document_id}`と、サーバの`data/audio/document-{document_id}`に
...
原音のsync間隔は暫定1秒分。グラスの実負荷・容量不足時の写真停止・最後の写真確認からの復元は未検証です。
