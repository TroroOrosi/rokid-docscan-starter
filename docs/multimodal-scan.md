...
未ACK写真と送信先IDは再起動後も保持します。リスニング開始は現状まだASR準備確認とサーバ文書作成を待ちます。

速い別々のKeyEvent操作の抑止と、大問／小問メニューのBACKを一段戻りへ修正しました。
最後の写真確認中の第2終了操作、確定後に届く期限内の取消は未接続です。
録音末尾の回収と未ACK再送、最後の写真確認が残る状態からの音声復元は実装しました。
有効サンプル後のRECと、写真確認を保ったままの録音終了はRP-08で接続中です。
...
| 撮影終了・録音継続 | — | この状態へ遷移した後に録音終了、残りのASRを待つ | — |
