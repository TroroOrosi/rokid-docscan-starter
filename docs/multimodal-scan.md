...
以下は現配布実装の説明です。[再整理した計画](../tasks/plan.md)の起動二択と即開始はまだ接続されていません。
現状は起動設定の録音フラグで開始し、ASR準備確認とサーバ文書作成を待ちます。
写真送信は次ページの進行を待たせ、確定ページを複数ためて再起動後に送る機能はありません。
速い別々のダブルタップが重複扱いで抑止されること、最後の写真の確認中に第2終了操作が音声へ届かないことを
ソース確認で見つけています。下の表はこの制限を含む実装上の操作です。物理受け入れ済みではありません。

また、答案取得は現在一度だけで、完成した小問の追加反映は未接続です。大問／小問メニューのBACKも
現在は一段戻らず終了判定へ進みます。起動後の任意の前回答案、全phaseのCLOSED保存、終了後の再装着自動起動は
今後の接続範囲です。詳細とソース根拠は[照合表](requirements-audit.md)を参照してください。
