...
| U8 | 検証（バリデーション）実行 | 読取サンプルで精度を確認（照合 `/match` はテキスト照合が主。`scripts/evaluate.py` は互換の画像経路のチューニング用） | `scripts/evaluate.py` の accuracy を確認 |
...
1. POST /v1/documents + 全ページ POST /v1/documents/{id}/pages
   （中断したら GET /v1/documents/{id}/scan-status?expected_total_pages=N で欠番を確認 → 欠番のみ再読取）
2. POST /v1/documents/{id}/finalize  ← 完了宣言（忘れずに）
3. 視認中ページの認識テキスト（その場認識） → POST /v1/match (ocr_text) → HUD に PAGE/LOW_CONF/NO_PAGE 表示
   （image は互換の任意入力・非推奨。撮影しない主経路では送らない）
```
