...
2026-08-28に実写を測った結果、**読めない原因は解像度ではありません**。列ピッチは
37pxあり、ML Kitの下限16px（および「24pxを超えると効果が無い」）を大きく上回って
います。不足しているのはコントラスト（41/255・16%。スキャンは約200）とJPEGの
情報量（0.071バイト/画素。文書は0.2〜0.5）です。したがってsweepは上限探索ではなく、
**変数を1つずつ分離する3枚**として行います。旧計画の`4032x3024 q50`は画素を5.8倍に
しながら品質を80から50へ下げるため、実行しません。測定の詳細は
[capture-timing-findings.md](capture-timing-findings.md)にあります。
スマホ画面の幅・高さ・品質は実行時に変更でき、`撮影設定を適用`または
`次のプリセット`で再ビルドなしに切り替えられます。値は保存され、
不正値は既定へ戻ります。プリセットの順序は次のとおりです。

| # | 設定 | 照明 | 変える変数 | この試行で分かること |
|---|---|---|---|---|
| 1 | `1920x1080 q80` | 通常 | ―（基準） | 既測: 147,398バイト・5.2秒・OCR671文字・0.071バイト/画素 |
| 2 | `1920x1080 q95` | 通常 | JPEG圧縮 | バイト/画素が0.2を超えるか。超えてOCRが伸びれば原因は圧縮 |
| 3 | `1920x1080 q80` | 強く均一 | コントラスト | 41/255が改善するか。改善してOCRが伸びれば原因は照明 |
| 4 | `4032x3024 q80` | 通常 | 解像度 | #2と#3で足りないときだけ。既知NGの再確認を兼ねる |

#### 手順

1. #1で1枚撮り、スマホのログに `1920x1080 q80 …KB 予算…%` が出ることを確認する。
   すでに実測値があるので、これは基準が再現するかの確認である。
2. `次のプリセット`で#2（`1920x1080 q95`）へ進め、同じ用紙をもう1枚撮る。
   変わるのは圧縮だけで、画角も距離も照明も#1と同じにする。
3. `撮影設定を適用`で`1920x1080 q80`へ戻し、照明を強く均一にして#3を撮る。
   ここで変わるのは照明だけで、設定は基準と同じにする。
4. 結果を次のとおり読む。
   - **#2でバイト/画素が0.2を超え、OCR文字数か確信度が伸びた**: 原因は圧縮。
     常用の品質を上げる。
   - **#3で伸びた**: 原因は照明であり、公開SDKには露出も焦点も無いのでアプリでは
     直せない。撮影姿勢と環境の要件として記録する。
   - **どちらも伸びない**: そのときだけ#4（`4032x3024 q80`）へ進む。
     `応答なし 30.0s` なら、ファームウェアが解像度自体を拒否している。
5. 採用した設定と、そのときのCXR-Lバージョン（接続時に
...
グラスのカメラは109°の超広角です。**距離で結論が変わるので、どちらの場合を
話しているかを先に言うこと。** 公称運用距離の40〜60cmで撮ったA4は横幅の約23%しか
占めず、10.5ptの文字は1920x1080では1文字あたり5〜9pxにしかならず、ML Kitが要求する
16pxを割ります。一方、2026-08-28に実測した近距離の1枚では用紙が横幅いっぱいに写り、
列ピッチは37pxありました。この距離では解像度は制約になりません。0文字が出たときは、
解像度を上げる前にまず撮影距離を確認します。
