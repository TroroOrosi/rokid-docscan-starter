...
```
認識テキスト ─▶ layout(構造化＋解答欄box) ─▶ subjects(科目推定)
        │                                          │
...
- **solve**: `retrieval.retrieve_context(documents/pages)` で根拠 `context`/`evidence_pages` を取得→`Question(context=..., image_path=None)` に注入→`solve_with_fallback`→`solutions` に保存。既存DBに画像パスがあってもAPI経路はモデルへ渡さない。
  - 図・数式・表・選択肢は端末側認識結果 `vision_text` を本文へ統合して扱う。
...
**主経路**。クライアントは公式SDKで非記録認識を確認できた場合だけ認識テキストを送り、読取完了時に
認識セッションを閉じる。サーバは物理カメラ・LEDを観測せず、消灯済みとは断定しない。

```
フェーズ1 読取（非記録認識。画像・動画・生フレームなし）
  端末内認識テキスト ─▶ POST /pages(ocr_text, vision_text) ×N ─▶ finalize
  ダブルタップ ─▶ POST /finalize-reading ─▶ segment_problems(全ページ) ─▶ questions 行 ×問題数
                                            status: open/reading → reviewing
フェーズ2 解答（視覚センサー不要・一括）
...
- **status ライフサイクル**：`open`（既定・読取フェーズの別名）→ `reviewing`（`finalize-reading` で
  遷移）。`finalize-reading` は**冪等**。応答の `camera` はクライアントへ
  `close_visual_sensor` を要求し、`observed_state:"not_observed_by_server"` と明示する。
...
登録済み認識テキストだけを参照し、サーバはカメラ入力を要求しない。
...
- **テキスト限定取り込み**：`POST /v1/documents/{id}/pages` は認識結果を**テキスト**で受ける：
  `ocr_text`（本文）＋`vision_text`（**図・グラフ・写真・見た目の読み取り**）。画像は送らない（`image_path=NULL`・
  `phash=""`・`ocr_md5` はテキストから算出）。入力は `application/json` のみ。multipart は本文解析前に
  `415`、JSON内の未知フィールドは `422`、両テキストが空なら `400`。
...
- **グラス単独操作**：`OPERATION_CONTRACT`（`GET /v1/settings.operations`）が 3 フェーズの全操作
  （`recognize_page`/`finish_reading`/`mode_toggle`/`audio_record_toggle`/`review_next_problem`/
...
| モジュール | 役割 | 差込口 |
|------------|------|--------|
| `app/solvers/`（base/registry/local_placeholder/**claude=LLMSolver**） | 問題解答ポート。API経路は認識テキストのみを渡し、既存画像パスを無視。実アダプタ openai/gemini/claude 同梱 | `ROKID_SOLVER=openai\|gemini\|claude`＋各社 API キー。`ROKID_SOLVER_TIERS` で tier 指定 |
...
- **ページ照合**は `app/matching.py: match_text`（OCR-MD5＋近似テキスト類似度）。
...
| GET | `/v1/exam-sessions/{id}/current` | 現在ページ把握（二次経路。科目・プレビュー。旧画像はignored表示） |
| POST | `/v1/exam-sessions/{id}/solve-current` | 現在ページを解く（二次経路。listening 時は書き起こしを統合）→ `glasses_view` |
| POST | `/v1/exam-sessions/{id}/questions` | `ocr_text`/`vision_text`＋任意bbox → 構造化・科目推定・`media`抽出。画像は415 |
...
- 読取信頼度（`read_conf`）＝認識テキストが十分か。空入力は登録前に400。
...
`OVERLAY_CONTRACT_VERSION` を契約ごとに管理。`API_VERSION` は現在 `1.9.0`、`APP_VERSION` は `0.9.0`。
...
実機 Rokid Glasses は両眼モノクロ緑Micro-LEDの情報表示デバイスです。公開仕様とSDK挙動は
[cxr-l-integration.md](cxr-l-integration.md) の確認区分に従います。
