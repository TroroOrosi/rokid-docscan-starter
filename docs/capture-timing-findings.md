...
1. **`0.3.9`（versionCode 14）を実機へ入れ、タップが届くことを物理確認する。**
   グラスをタップし、logcat に `userInitiated=true` が出ること、
   ビュー切替の直後に意図しないシャッターが落ちないことの両方を見ます。
   これが取れるまで、他の撮影側の変更は入れません。
2. **撮影パラメータの sweep**（未実施）。上の「計画の訂正」の 3 枚を、変数を
   1 つずつ分離して撮ります。`SWEEP_PRESETS` はこの順序（基準 → `1920x1080 q95`
   → `4032x3024 q80`）へ修正済みです。照明を変える 2 枚目は設定ではなく撮影条件
   なので、プリセットではなく基準を撮り直します。公開 API は
   `takePhoto(width, height, quality)` だけで、対応解像度の一覧は
