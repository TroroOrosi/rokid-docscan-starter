...
- **期待結果**: 両眼 HUD に音・白フラッシュの指示なしで3行が表示される。**480×398/眼 での折返し・
...
### C-1. 撮影完了後のカメラ/LED実状態の確認

- **現状の前提**: `GET /v1/settings.capture` と finalize-reading 応答は、登録後の解答・閲覧で
  新規撮影を要求しないことと、クライアントへの期待状態を示す。サーバは物理カメラ/LEDを制御・検証しない。
- **実機手順**: 外部カメラ（スマホでよい）でグラス前面を録画しながら、ページ画像の撮影
  （2本指タップ）→ scan-status確認 → finalize-reading → 閲覧、と一巡する。
  タイムスタンプ付きでカメラ/LED挙動を記録する。
- **期待結果**: 各ページの撮影時に端末仕様どおりのプライバシー表示が働き、登録完了後は
  クライアントが新規キャプチャを開始しない。物理LEDの消灯時点は実測結果として記録する。
- **反映**: カメラが不要に動作し続ける場合はクライアント側を修正する。端末/ファーム固有の
  インジケータ挙動は保証値にせず、検証結果として docs に注記する。
...
B-4 UX 所感: ____
C-1 camera/LED実測: 撮影時 ____ / finalize-reading 後 ____（動画: ____）
