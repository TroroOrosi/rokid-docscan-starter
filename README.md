...
3. `CAPTURE_REVIEW` の縮小プレビューで未登録写真を確認し、スマホの
   「この写真を登録」で登録する。グラスのタップなら同じページの再撮影準備へ戻る。
...
現在のバージョン: **Server APP 0.15.0 / API 1.13.1 / Android client 0.3.0 /
...
| 撮影確認 (`CAPTURE_REVIEW`) | 同じページの `AIMING` を開始 | **この写真を登録** / 同じページを撮り直す / 未登録写真を破棄 |
